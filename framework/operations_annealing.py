...
def minimize_witnesses(witness_classes, rho=None, counts=None, num_guesses=10):
    ...
    def optimize(W_val, witness_idx, bounds):
        """
        ...
        min_W = dual_annealing(loss, bounds)
    ...
    for class_idx, W_class in zip([3, 5, 7, 8], all_expec_vals):
        ...
        low_bounds = [lower_bound]*num_params
        upper_bounds = [theta_bound, alpha_bound, beta_bound, gamma_bound][:num_params]
        bounds = Bounds(lb=low_bounds, ub=upper_bounds)

        for witness_idx in range(1, num_witnesses+1): # witnesses are indexed from 1            
            best_param, min_val = optimize(W_class, witness_idx, bounds)
