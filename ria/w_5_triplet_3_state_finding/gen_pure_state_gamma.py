...
def analyze_rhos(rho_actuals, id='id'):
    ...
    df.to_csv(f'{id}_analysis.csv')

def make_plots_E0(dfname, fig_title, chi_vals):
    ...
    fig.tight_layout()
    fig.savefig(f'{fig_title}_analysis_plotted.pdf')
    fig.show()
...
def get_theo_rho(state, chi, gamma):
    ...
    if state == 'HR_VL_gamma':
        phi = np.cos(chi/2)*np.kron(H,R) + np.exp(1j * gamma) * np.sin(chi/2) * np.kron(V,L)
        
    if state == 'HL_VR_gamma':
        phi = np.cos(chi/2)*np.kron(H,L) + np.exp(1j * gamma) * np.sin(chi/2) * np.kron(V,R)
            
    if state == 'HD_VA_gamma':
        phi = np.cos(chi/2)*np.kron(H,D) + np.exp(1j * gamma) * np.sin(chi/2) * np.kron(V,A)
            
    if state == 'HA_VD_gamma':
        phi = np.cos(chi/2)*np.kron(H,A) + np.exp(1j * gamma) * np.sin(chi/2) * np.kron(V,D)

    if state == 'HV_VH_gamma':
        phi = np.cos(chi/2)*np.kron(H,V) + np.exp(1j * gamma) * np.sin(chi/2) * np.kron(V,H)
...
if __name__ == '__main__':
    ...
    # analyze rho files
    analyze_rhos(rho_actuals, id=STATE_ID)
    make_plots_E0(f'{STATE_ID}_analysis.csv', STATE_ID, chis)